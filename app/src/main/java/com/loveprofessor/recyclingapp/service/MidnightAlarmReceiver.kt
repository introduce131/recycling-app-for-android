...
class MidnightAlarmReceiver : BroadcastReceiver() {
    private val METER_PER_STEP = 0.7    // 100걸음에 70m, 1보당 70cm
    private val KM_PER_CAR = 166.0      // 자동차 1KM 주행시 탄소 배출량 166g
    ...
    private fun saveStepCount(context: Context, stepCount: Int) {
        ...
        val reportData = hashMapOf(
            ...
            "report_step_data" to stepCount,
            "report_carbon_data" to calculateCarbon(stepCount) // 탄소 절감량 계산
        )
        ...
        db.collection("steps_report")
            ...
            .addOnSuccessListener {
                // 데이터 저장 성공 시 로그 출력
                Log.d("MidnightReceiver", "걸음 수 데이터 저장 성공: $stepCount")
    ...
    private fun resetStepCount(context: Context) {
        val prefs: SharedPreferences = context.getSharedPreferences("step_prefs", Context.MODE_PRIVATE)
        prefs.edit().putInt("step_count", 0).apply()  // 걸음 수 초기화
    }

    // 탄소 저감량을 계산함
    private fun calculateCarbon(stepCount: Int): Float {
